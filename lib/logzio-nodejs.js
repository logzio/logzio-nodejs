...
var LogzioLogger = function (options) {
    ...
    if (options.protocol && ['http', 'https', 'udp'].indexOf(options.protocol) === -1)
        throw new Error('Invalid protocol defined. Valid options are http, https or udp');
    ...
    this.port = this.protocol === 'udp' ? 5050 : (this.protocol === 'http' ? 8070 : 8071);
...
LogzioLogger.prototype._sendUDP = function() {
    ...
    for (var i=0; i<messagesLength; i++) {
        ...
        this.udpClient.send(buff, 0, buff.length, this.port, this.host, function(err, bytes) {
            if (err) 
                throw new Error('Failed to send udp log message. err = ' + err);
        });
    }
};
