...
LogzioLogger.prototype._defaultCallback = function (err) {
    if (err && !this.supressErrors) {
        try {
            ...
            console.log('(Using console logger)logzio-nodejs: ' + err, err);
        }
...
LogzioLogger.prototype._debug = function (msg) {
    if (this.debug) {
        try {
            this.internalLogger.log('logzio-nodejs: ' + msg);
        } catch (e) {
            console.log('(Using console logger)logzio-nodejs: ' + msg);
        }
    }
};
            ...
            if (err) {
                ...
                if (err.code === 'ETIMEDOUT' || err.code === 'ECONNRESET' || err.code === 'ESOCKETTIMEDOUT' || err.code === 'ECONNABORTED') {
                    if (bulk.attemptNumber >= self.numberOfRetries) {
                        self.callback(new Error('Failed after ' + bulk.attemptNumber + ' retries on error = ' + err, err));
                    } else {
                ...
                if (responseCode !== '200') {
                    self.callback(new Error('There was a problem with the request.\nResponse: ' + responseCode + ': ' + body.toString()));
                } else {
